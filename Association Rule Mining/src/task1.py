...
def main(argv):
    case_number = int(argv[1])  # case 1 for frequent businesses, case 2 for frequent users
    support_threshold_total = int(argv[2])
    input_file_path = argv[3]
    output_file_path = argv[4]

    input_file_path = "/Users/zhijunliao/Marks/USC/INF-553/HW/INF553HW2/data/small2.csv"
    output_file_path = "/Users/zhijunliao/Marks/USC/INF-553/HW/INF553HW2/data/firstname_lastname_task1.txt"
    output_file_path = "/Users/zhijunliao/Marks/USC/INF-553/HW/INF553HW2/data/task1_case_2_threshold_4.txt"
    case_number = 2
    support_threshold_total = 6
    ...
    def qualified_as_candidate_pair(pair: list, frequent_itemsets: dict) -> bool:
        for item in pair:
            try:
    ...
    def son(baskets: RDD, support_threshold_total=support_threshold_total) -> list:
        ...
        def pcy_for_list(partition: list, support_threshold_total=support_threshold_total) -> dict:
            ...
            for basket in partition:
                # pairs = generate_combination(basket, size=2)
                pairs = generate_combination_with_filter(basket, frequent_itemsets, size=2)
                for pair in pairs:
                    # if qualified_as_candidate_pair(pair, frequent_itemsets):
                    key = tuple(pair)
    ...
    def pcy_for_rdd(baskets: RDD, support_threshold_total=support_threshold_total) -> list:
        ...
        while True:
            ...
            frequent_itemsets = baskets.\
                flatMap(lambda basket: generate_combination_with_filter(basket, frequent_itemsets_dict, current_itemset_size)). \
    ...
    data = sc.textFile(input_file_path).filter(lambda pair: "user_id" not in pair).map(lambda string: string.split(","))
    if case_number == 1:  # find frequent businesses, 19 baskets in total
        baskets = data.groupByKey().map(lambda pair: sorted(list(set(pair[1].data)))).persist()  # set() is to ensure uniqueness
    if case_number == 2:  # find frequent users, 12 baskets in total
        baskets = data.map(lambda pair: (pair[1], pair[0])).groupByKey().map(lambda pair: sorted(list(set(pair[1].data)))).persist()

    frequent_itemsets = son(baskets, support_threshold_total)
